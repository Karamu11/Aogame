...
def get_page(cookie):
    ...
    for b in a:
        ...
        listuid.append(uid)
        name = b['profile']['name']
    print (f" Tìm Thấy {sl} Page Profile ")
def get_data(cookie):
    headers = {
        'authority': 'mbasic.facebook.com',
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
...
def login_tds(token):
	try:
		...
		if 'success' in r:
			...
			print(" Token TDS Không Đúng, Hãy Kiểm Tra Lại!\n")
			return 'error_token'
	except:
		return 'error'
...
def cau_hinh(id, TDS_token):
	...
	try: 
	    ...
	    print(f" Cấu Hình Thành Công ID : {id}  ")
	except:
...
def chon_job(so,chon):
	if chon == 1 :
		...
		if so == 7 :
			...
			type = ['like','likegiare','likesieure','reaction','group','follow','follow']
...
token = input(' \033[1;97mNhập Token TDS : ')
check_xu = login_tds(token)
cookie = input(' Nhập Cookie Facebook : ')
print('-'*60)
#### vào việc
print (" Nhập [ 1 ] Job Like + Cảm Xúc")
print (" Nhập [ 2 ] Job Group")
print (" Nhập [ 3 ] Job Follow ")
print (" Nhập [ 4 ] Job Group + Like + Cảm Xúc")
print (" Nhập [ 5 ] Job Group + Like + Cảm Xúc + Follow ")
chon_1 = int(input(' Nhập : '))
print('-'*60)
dl = int(input(' Nhập Delay : '))
doi_tt = int(input(' Chạy Bao Nhiêu Job Thì Đổi PROFILE  : '))
os.system('clear')
logo()
get_page(cookie)
tt = 0
while True :
	for id_page in listuid:
		...
		while True :
			try:
				if doi_a == doi_tt  :
					print (' ĐANG ĐỔI PRO5 TIẾP THEO ',end='\r')
					break
				elif ch == 'o' :
					break
				else :
						...
						try :
							...
							for job_2 in a:
								...
								try :
									...
									print (f' {type_1} | ERRO => {id_job}',end='\r')
									sleep(1)
								except :
									tt += 1
									doi_a += 1
									...
									(f" [{tt}] <> {type_1} <> {id_job} <> {xu_1} <> {xu_2} Xu")
									if doi_a == doi_tt : 
										print (' ĐANG ĐỔI PRO5 TIẾP THEO ',end='\r')
										break
									for i in range(dl,-1,-1):
										print(f'[ Đợi Sau ] => {i} GIÂY',end='\r')
										sleep(1)
			except:pass

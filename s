...
class Facebook_Api (object):
	...
	def share(self, id, nd="Ngọc Tool"):
		try:
			url = requests.get('https://mbasic.facebook.com/'+id, headers=self.headers).url
...
def error(id, type):
	time=datetime.now().strftime("%H:%M:%S")
	uid = id.split('_')[1] if '_' in id else id
	print(f'[X] | {time} | {type} | {uid} | ERROR', end = '\r'); sleep(2); print('                                                   ', end = '\r')
...
def main():
	...
	nhiemvu = input ('~[●] ➩ Nhập Số Để Chạy Nhiệm Vụ: ')
				...
				if nvgroup == 1:
					...
					if listgroup == False:
						...
						if len(listgroup) == 0:
							...
							print(f'Tìm Thấy {len(listgroup)} Nhiệm Vụ Group           ', end = '\r')
